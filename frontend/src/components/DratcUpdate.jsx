...
// Component για ενημέρωση υπάρχοντος record στον πίνακα DRATC
function DratcUpdate({ token }) {
  // Παίρνουμε το drugid και το atcCode από το URL
  const { drugid, atcCode } = useParams();

  // State για τη φόρμα δεδομένων
  const [formData, setFormData] = useState({
    drugid: '',    // ID φαρμάκου
    atcCode: ''    // Κωδικός ATC
    // Προσθέστε άλλα πεδία του πίνακα DRATC αν χρειάζεται
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log(`Ανάκτηση δεδομένων από /dratc/${drugid}/${atcCode}`);
        const response = await api.get(`/dratc/${drugid}/${atcCode}`);
        console.log('Απάντηση δεδομένων:', response.data);
        setFormData(response.data); // Ενημέρωση του state
      } catch (err) {
        console.error('Σφάλμα κατά την ανάκτηση δεδομένων DRATC:', {
          message: err.message,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log(`Ενημέρωση δεδομένων σε /dratc/${drugid}/${atcCode} με δεδομένα:`, formData);
      await api.put(`/dratc/${drugid}/${atcCode}`, formData);
      // Επιτυχής ενημέρωση: πλοήγηση στη σελίδα αναζήτησης
      navigate('/search');
    } catch (err) {
      console.error('Σφάλμα κατά την ενημέρωση δεδομένων DRATC:', {
        message: err.message,
        status: err.response?.status,
        data: err.response?.data,
      });
  ...
  return (
    ...
    <Box sx={{ p: 2, maxWidth: 600, mx: 'auto' }}>
      {/* Τίτλος της σελίδας */}
      <Typography variant="h4" gutterBottom>Ενημέρωση DRATC</Typography>
