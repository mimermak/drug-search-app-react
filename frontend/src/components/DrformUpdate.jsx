...
// Component για ενημέρωση υπάρχοντος record στον πίνακα DRFORM
function DrformUpdate({ token }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log(`Ανάκτηση δεδομένων από /drform/${drugid}`);
        const response = await api.get(`/drform/${drugid}`);
        console.log('Απάντηση δεδομένων:', response.data);
        setFormData(response.data); // Ενημέρωση του state
      } catch (err) {
        console.error('Σφάλμα κατά την ανάκτηση δεδομένων DRFORM:', {
          message: err.message,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log(`Ενημέρωση δεδομένων σε /drform/${drugid} με δεδομένα:`, formData);
      await api.put(`/drform/${drugid}`, formData);
      // Επιτυχής ενημέρωση: πλοήγηση στη σελίδα αναζήτησης
      navigate('/search');
    } catch (err) {
      console.error('Σφάλμα κατά την ενημέρωση δεδομένων DRFORM:', {
        message: err.message,
        status: err.response?.status,
        data: err.response?.data,
      });
  ...
  return (
    ...
    <Box sx={{ p: 2, maxWidth: 600, mx: 'auto' }}>
      {/* Τίτλος της σελίδας */}
      <Typography variant="h4" gutterBottom>Ενημέρωση DRFORM</Typography>
